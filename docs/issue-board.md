...
| 2 | Build the eligibility questionnaire and escalation rules. | Done | `web/shared/eligibility.js` implements the eight questions and five derived and completeness checks, across fourteen reason codes on the `professional_review` and `operator_action` routes. Answers persist in `case_eligibility` (`case_id`, `answers_json`, `assessed_at`); no outcome is stored. `PUT /api/cases/:id/eligibility` saves them, scoped to the owning operator. The outcome is recomputed in the browser from the stored answers plus a live agreement read on every open, and a due-date-versus-deadline mismatch is always visible in the outcome banner. 12 fixture tests, 2 new store tests, and 1 new route test pass; a Chrome browser check exercised the unanswered, supported, and dispute-escalation states, persistence across a reload, and the mismatch banner, but never exercised the unreadable-agreement path. |
