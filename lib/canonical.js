 ...
 *
 * UNVERIFIED: this encoding is inferred from the Flare interface docs and has NOT
 * yet been confirmed against a real attestation response. Confirm it by running
 * scripts/xrpl-spike.js, requesting an XRPPayment attestation for the resulting
 * transaction, and checking that the returned `receivingAddressHash` equals this
 * value. Until then, do not rely on a destination match.
 */
function standardAddressHash(xrplAddress) {
  return keccak256(
    AbiCoder.defaultAbiCoder().encode(["string"], [String(xrplAddress).trim()])
  );
}
